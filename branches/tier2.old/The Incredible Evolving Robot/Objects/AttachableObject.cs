    ...
    public override void Explode(BasicObject parent)
    {
      ...
      foreach (Connector conn in this.Connectors)
      {
		...
		public virtual void UpdateVelocity(AttachableObject parent, Vector3 velocity)
		{
			...
			this.spawnMatrix *= Matrix.CreateTranslation(velocity);
		}
		...
		public virtual int UpdateHealth(AttachableObject parent)
		{
			...
			this.Exploded = false;
			return totalHealth + this.Health;
		}

    public override void UpdateBoundingObjects()
    {
      for (int i = 0; i < this.BoundingSphereMetas.Count; i++)
        this.BoundingSphereMetas[i].Center = this.Position.Coordinate
          + Vector3.Transform(this.BoundingSphereMetas[i].Offset, (Matrix.CreateFromQuaternion(this.Position.Front) * this.SpawnMatrix));
      for (int i = 0; i < this.BoundingBoxMetas.Count; i++)
        this.BoundingBoxMetas[i].Center = this.Position.Coordinate
          + Vector3.Transform(this.BoundingBoxMetas[i].Offset, (Matrix.CreateFromQuaternion(this.Position.Front) * this.SpawnMatrix));
      for (int i = 0; i < this.BoundingBarMetas.Count; i++)
        this.BoundingBarMetas[i].Center = this.Position.Coordinate
          + Vector3.Transform(this.BoundingBarMetas[i].Offset, (Matrix.CreateFromQuaternion(this.Position.Front) * this.SpawnMatrix));
    }

    public override void Update(GameTime gameTime)
    {
      if (this.Health < 0 && !this.Exploded)
        this.Explode(this.Parent);
